...
import { getConfig, pickMod, queryModinfo, RegisteredModList, updateConfig } from './utils';
import { CONFIGS, COMMANDS, LOGGER, resolvePath, ModFile, ModInfo } from './types';

const { executeCommand } = commands;
const { showErrorMessage, showWarningMessage, showOpenDialog } = window;
const { keys } = Object;

export async function cloneFile(source: ModFile, destination: ModFile, overwrite = false) {
    try {
        LOGGER.debug(`cloneFile: Copying from "${resolvePath(source)}" to "${resolvePath(destination)}"...`);
        await mkdir(`${destination.modPath}/${dirname(destination.path)}`, { recursive: true });
        await copyFile(resolvePath(source), resolvePath(destination), overwrite ? 0 : fsConstants.COPYFILE_EXCL);
    } catch(error: any) {
        if(error.code === 'EEXIST') {
            LOGGER.info(`cloneFile: File "${destination.path}" already exists in mod filesystem at "${destination.modPath}". Prompting user to overwrite...`);
            ...
            LOGGER.error(`cloneFile: Encountered unexpected error while copying file`, error);
...
export async function diffFile(source: ModFile, destination: ModFile) {
    if(resolvePath(source) === resolvePath(destination)) {
        ...
        LOGGER.error(`diffFile: User somehow triggered self-diff on "${resolvePath(source)}", aborting diff...`);
...
async function registerModImpl(modinfo?: ModInfo) {
    ...
    if(isFirstTime) {
		...
		if(!uris) {
            LOGGER.debug("registerMod: No folder selected, user must have canceled...");
			return;
        }
		modPath = uris[0].fsPath;
    }
    else {
        LOGGER.debug(`reregisterMod: Mod "${modinfo.modName}" is located at "${modinfo.modPath}"`);
    ...
    if(name) {
        if(config) {
            ...
            if(!isFirstTime) {
                if(name != modinfo.modName) {
                    LOGGER.info(`reregisterMod: Deleting old mod entry for ${modinfo.modName}...`);
                    config = keys(config)
                    ...
                    LOGGER.info(`reregisterMod: ${modinfo.modName} still has the same name, no change is needed...`);
                    return;
                }
            }
        } else {
        ...
        LOGGER.info(`${isFirstTime ? "" : "re"}registerMod: Updating config ${CONFIGS.REGISTERED_MODS}[${name}] to "${modPath}"...`);
        await updateConfig(CONFIGS.REGISTERED_MODS, config);
        return true;
    } else if(isFirstTime) {
        LOGGER.info(`registerMod: Invalid or missing modinfo at ${modPath}, retrying...`);
        ...
        LOGGER.error(`reregisterMod: Invalid or missing modinfo for ${modinfo.modName}!`)
        await showErrorMessage(`Mod ${modinfo.modName} no longer exists! It may have been moved or deleted.`, { modal: true });
        return;
    }
}