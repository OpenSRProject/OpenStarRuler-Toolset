...
export function buildBaseFileOperation(
    ...
    return async () => {
        ...
        if (!file) {
            LOGGER.info(
                'baseFileOperation: Source file not found, aborting...',
            );
            return;
        }
...
export function buildAncestorFileOperation(
    ...
    return async () => {
        ...
        if (!file) {
            LOGGER.info(
                'ancestorFileOperation: Source file not found, aborting...',
            );
            return;
        }
        operation(file, { ...modinfo, path: file.path });
...
export function buildModFileOperation(
    ...
    const fileFinder = forCurrent
        ? findForCurrentFile(findModFile)
        : findModFile;
    return async () => {
        const destinationMod = await getDestinationMod(forCurrent);
        if (!destinationMod) {
            LOGGER.info(
                'modFileOperation: No destination mod selected, aborting...',
            );
            return;
        }
        ...
        if (!sourceMod) {
            LOGGER.info(
                'modFileOperation: No source mod selected, aborting...',
            );
            return;
        }
