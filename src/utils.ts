...
async function safeStat(file: string): Promise<Stats | undefined> {
	try {
		...
		LOGGER.error(`safeStat: Encountered unknown error fetching stats for "${file}"`, error);
		throw error;
	}
}
...
export async function pickModinfo(modinfos: ModInfo[], title: string): Promise<ModInfo | undefined> {
	const modinfo = await showQuickPick(
		modinfos.map((modinfo) => {
			LOGGER.debug(`pickModinfo: Option "${modinfo.modName}" maps to "${modinfo.modPath}"`);
			return {
...
export async function getDestinationMod(): Promise<ModInfo | undefined> {
	...
	if(modinfos.length === 1) {
		...
		if(modinfo)
			LOGGER.info(`getDestinationMod: User selected "${modinfo.modName}" at "${modinfo.modPath}"`);
		return modinfo;
	}
...
export async function queryModinfo(pathToMod: string, field: ModinfoField): Promise<string | undefined> {
    const modinfoPath = pathToMod.endsWith(MODINFO) ? pathToMod : joinPath(pathToMod, MODINFO);
    try {
        const modinfo = await readFile(modinfoPath, { encoding: "utf8" });
		LOGGER.trace(modinfo);
        return modinfo.match(new RegExp(`^[ \t]*${field}[ \t]*:[ \t]*(.*?)[ \t]*$`, "im"))?.[1];
    } catch (error: any) {
        if(error.code === 'ENOENT') {
			LOGGER.error(`queryModinfo: Modinfo not found at ${modinfoPath}!`);
            return undefined;
		}
		LOGGER.error(`queryModinfo: Encountered unknown error reading modinfo at "${modinfoPath}"`, error);
        throw error;
    }
}

export async function getGamePath(purpose: string, isInternal?: boolean): Promise<string | undefined> {
	const gamePath = getConfig(CONFIGS.BASE_GAME_FOLDER) as string;
	if(!gamePath) {
		if(isInternal) {
			LOGGER.info("getGamePath: No game path in config, but isInternal is set. Aborting...");
			return;
		}
		else {
			LOGGER.info("getGamePath: No game path in config. Prompting user for game path...")
			...
			LOGGER.info("getGamePath: findBaseFiles returned undefined, user must have canceled...");
			return;
		}
	}
	return gamePath;
...
export async function requestFilePath(purpose: string, defaultPath?: string, isInternal?: boolean): Promise<string | undefined> {
...
export async function findBaseFile(purpose: string, gamePath?: string, usePath?: string, isInternal?: boolean): Promise<ModFile | undefined> {
	if(!gamePath) {
	...
	if(!(await safeStat(resolvePath(file)))?.isFile()) {
		if(isInternal) {
			LOGGER.info(`findBaseFile: File "${path}" does not exist in "${gamePath}", but isInternal is set. Aborting...`);
			return;
		} else {
			LOGGER.info(`findBaseFile: File "${path}" does not exist in "${gamePath}". Retrying...`);
...
export async function pickMod(purpose: string, modName?: string): Promise<ModInfo | undefined> {
	const registeredMods = getConfig(CONFIGS.REGISTERED_MODS) as RegisteredModList;
	if(modName) {
		LOGGER.debug(`pickMod: Autoselecting "${modName}"...`);
		return registeredMods ? { modName, modPath: registeredMods[modName] } : undefined;
	}
	if(!registeredMods?.length) {
		LOGGER.info(`pickMod: No registered mods, prompting user to register one...`);
	...
	if(modinfos.length === 1) {
		LOGGER.info(`pickMod: Only one registered mod, defaulting to "${modinfos[0].modName}" at "${modinfos[0].modPath}"`);
		return modinfos[0];
	} else {
		const modinfo = await pickModinfo(modinfos, "Select Source Mod");
		if(modinfo)
			LOGGER.info(`pickMod: User selected "${modinfo.modName}" at "${modinfo.modPath}"`);
		else LOGGER.info("pickMod: No mod selected, user must have canceled...");
		return modinfo;
	}
}

export async function findAncestorFile(purpose: string, modName?: string, usePath?: string, isInternal = false): Promise<ModFile | undefined> {
    if(!modName) {
		LOGGER.info("findAncestorFile: No ancestor name provided, falling back to vanilla...");
        return await findBaseFile(purpose, await getGamePath(purpose, isInternal), usePath, isInternal);
	}
    const { modPath } = await pickMod(purpose, modName) || {};
    if(!modPath) {
		LOGGER.info(`findAncestorFile: Mod ${modName} not registered, falling back to vanilla...`);
        return await findBaseFile(purpose, await getGamePath(purpose, isInternal), usePath, isInternal);
    }
    const path = await requestFilePath(purpose, usePath, isInternal);
	if(!path) {
    ...
    if(!(await safeStat(resolvePath(file)))?.isFile()) {
        ...
        if(!ancestorFile) {
			if(isInternal) {
				LOGGER.info(`findAncestorFile: File "${path}" does not exist in "${modName}" or its ancestors, but isInternal is set. Aborting...`);
				return;
			} else {
				LOGGER.info(`findAncestorFile: File "${path}" does not exist in "${modName}" or its ancestors. Retrying...`);
                await showErrorMessage("The specified file does not exist! Are you sure you specified the right path?", { modal: true });
